...
class O2OStatModel(object):
                ...
                for lrate in XGB_learning_rate:
                    for l1_alpha in XGB_l1_alpha:
                        for l2_lambda in XGB_l2_lambda:
                            param = dict()
                            param = {'silent': 1, 'objective': 'binary:logistic', 'subsample': 1,
                                     'colsample_bytree': 1, "n_estimators": est}
    ...
    def due_overdue_reject_model(self, feature_path='data/train_feature_file.txt',
        ...
        best_param = self.xgbclass_add_reject(feature_file_path=feature_path,feature_type='reject', model_path=model_path2, model_type=model_type)
        # ''' 可以指定feature的路径, feature的label类型是"逾期"还是"拒绝+逾期" '''
        # feature_path = 'feature/data/feature_merged_recent.txt'
        feature_path = root + '/../feature/data/additional_feature_reject_overdue_{0}_{1}_xgb.txt'.format(today, model_type)
        feature_type = 'all'
    ...
    @exe_time
    def xgbclass_add_reject(self, feature_file_path=root + '/../feature/data/feature_merged_recent.txt', feature_type='overdue', model_path='', is_export_model_file=False,
        ...
        scaler = joblib.load(model_path + '_scaler.pkl')
        reject_feature_path = "../feature/data/additional_feature_reject_{0}_{1}_xgb.txt".format(today, model_type)
        reject_overdue_feature_path = "../feature/data/additional_feature_reject_overdue_{0}_{1}_xgb.txt".format(today,
                                                                                                             model_type)
        key_message_path = train_message_path
        try:
