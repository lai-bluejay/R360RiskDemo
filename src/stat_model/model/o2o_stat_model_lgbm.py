...
class O2OStatModel(object):
    ...
    def get_grid_search_params_list(self):
        ...
        """
        param_list = list()
        for est in XGB_estimators:
            for dep in XGB_tree_depth:
                for lrate in XGB_learning_rate:
                    for l1_alpha in XGB_l1_alpha:
                        for l2_lambda in XGB_l2_lambda:
                            param = dict()
                            params = {
                            'boosting_type': 'gbdt',
                            'objective': 'binary',
                            ...
                            'bagging_freq': 5,
                            'verbose': 0}
                            param['bst:max_depth'] = dep
                            param['bst:eta'] = lrate
                            param['bst:lambda'] = l2_lambda
                            param['bst:alpha'] = l1_alpha
    ...
    def dump_model_file(self, feature_label, best_param, model_path="",
        ...
        plst = list(best_param.items())
        clf = xgb.train(plst, dtr)
        if is_export_model_file:
    ...
    @exe_time
    def xgbclass_by_feature(self, feature_label, feature_type='overdue', feature_file_path='', model_path='',
                            model_type="majority"):
        """
        M1数据集上模型训练&选择参数，对于给定参数序列，输出每一组参数对应的训练集KS值
        :param rf_estimators: 随机森林中的决策树数量
        ...
        for param in self.param_list:
            ...
            dep = param['bst:max_depth']
            lrate = param['bst:eta']
            l2_lambda = param['bst:lambda']
            l1_alpha = param['bst:alpha']
            # param['num_feature'] = xgb_max_feature[i]
            # plst = param.items()
            params_string = str(param)
            print(params_string)
            # 给参数列表plst赋值,并根据dtr进行训练,结果保存到clf,再对dte进行测试,结果保存到result
            clf = xgb.train(param, dtr, verbose_eval=True)
            result = clf.predict(dva)
            ...
            with open(ks_record_path, 'a') as fo:
                string = 'xgbc test set %d %d %f %f %f %f %f \n' \
        ...
        best_ks_params_path = root + '/../output/xgb_best_ks_params_{0}.txt'.format(feature_type)
    ...
    def due_overdue_model(self, feature_path='data/train_feature_file.txt',
        ...
        best_param = self.xgbclass_by_feature(feature_label=feature_label, feature_type=feature_type,
                                              data_set_suffix='_o2o_1123.txt', model_path=model_path,
                                              model_type=model_type)
    ...
    def due_overdue_reject_model(self, feature_path='data/train_feature_file.txt',
        ...
        max_n_est, max_learning_rate, max_max_depth, max_score = self.xgbclass_add_reject(xgb_estimators=XGB_estimators,
        ...
        feature_label = feature_extractor.select_feature(feature_path, feature_type, begin=self.begin, end=self.end)
        if self._check_model_cached(model_path):
            self.dump_model_file(feature_label, xgb_tree_depth=max_max_depth, xgb_estimators=max_n_est,
                                 xgb_learning_rate=max_learning_rate, model_path=model_path)
        pass
    ...
    @exe_time
    def xgbclass_add_reject(self, xgb_estimators, xgb_max_feature, xgb_learning_rate, xgb_tree_depth,
                            feature_file_path=root + '/../feature/data/feature_merged_recent.txt', feature_type='overdue',
        ...
        max_n_est, max_learning_rate, max_max_depth, max_score = self.xgbclass_by_feature(overdue_reject_data,
                                                                                          xgb_estimators,
                                                                                          xgb_max_feature,
                                                                                          xgb_learning_rate,
                                                                                          xgb_tree_depth, feature_type,
                                                                                          feature_file_path=feature_file_path,
                                                                                          data_set_suffix=data_set_suffix,
                                                                                          model_path=model_path,
                                                                                          model_type=model_type)
        return max_n_est, max_learning_rate, max_max_depth, max_score
...
if __name__ == "__main__":
    ...
    # feature_type = 'overdue'
    # model_path = 'model_dir/model_o2o_stat_xgb_%s_model_{0}_{1}.pkl'.format(today, model_type)
    ...
    # end = '2016-10-15'
    # model = O2OStatModel(begin, end)
    # current_model_path = root + '/model_dir/model_o2o_stat_xgb_model_overdue_reject_0808.pkl'
    # minority_model_path = root + "/model_dir/model_o2o_stat_xgb_model_overdue_reject_{0}_minority.pkl".format(today)
    # #
    # '''验证模型在最新数据上的效果'''
    ...
    # '''现有线上模型的效果验证'''
    # # model_path = 'model/xgb_0225.pickle'
    # with open(vali_message_path, 'a') as fo:
    ...
    # end = '2016-10-31'
    # model = O2OStatModel(begin, end)
    # current_model_path = root + '/model/model_o2o_stat_xgb_model_overdue_reject_0808.pkl'
    # # han_model_path = root + "/model/model_o2o_stat_xgb_model_overdue_reject_2016-11-17_majority.pkl"
    # # minority_model_path = root + "/model/model_o2o_stat_xgb_model_overdue_reject_2016-11-17_minority.pkl"
    #
    # '''仅用于获取用于计算psi的数据集'''
    ...
    # model = O2OStatModel(begin, end)
    # current_model_path = root + '/model_dir/model_o2o_stat_xgb_model_overdue_reject_0808.pkl'
    # han_model_path = root + "/model_dir/model_o2o_stat_xgb_model_overdue_reject_{0}_majority.pkl".format(today)
    #
    # '''验证模型在最新数据上的效果'''
    # vali_feature_file = root + '/../feature/data/vali_feature_label.txt'
    ...
    # model = O2OStatModel(begin, end)
    # current_model_path = root + '/model_dir/model_o2o_stat_xgb_model_overdue_reject_0808.pkl'
    # # han_model_path = root + "/model_dir/model_o2o_stat_xgb_model_overdue_reject_2016-11-17_majority.pkl"
    # # minority_model_path = root + "/model_dir/model_o2o_stat_xgb_model_overdue_reject_2016-11-17_minority.pkl"
    #
    # '''仅用于获取用于计算ks, psi的数据集'''
